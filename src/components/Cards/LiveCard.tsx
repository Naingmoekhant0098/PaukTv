...
function LiveCard({item , setIsAdsShow,handleCurrentVideo} : itemProps) {
  ...
  <>
    ...
    <div className="rounded-[10px] border border-white/5 bg-white/5 backdrop-blur-[0.6px] w-[250px] min-h-[220px] md:min-h-[260px] md:w-[294px] p-4 md:p-6">
      ...
      {
          item && item.match_status==='live' ? <div onClick={()=>{setIsAdsShow(true),handleCurrentVideo(item,'match')}} className=" border p-3 px-8   rounded-full  text-center mt-2 md:mt-6 border-gray-400 text-gray-300 text-[13px] md:text-[14px]  transition-all duration-500 cursor-pointer hover:border-[#E9376E] hover:bg-[#E9376E] ">
          Watch Now
        </div>
        :
