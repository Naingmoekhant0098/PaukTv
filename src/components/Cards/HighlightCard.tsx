...
function HighlightCard({item,setIsAdsShow,handleCurrentVideo} : any) {
 ...
 <div className="absolute inset-0 flex items-center justify-center">
    <FaCirclePlay  
    onClick={()=>{setIsAdsShow(true),handleCurrentVideo({name : item?.snippet?.title , stream_url: `https://www.youtube.com/embed/${item?.id?.videoId}`},'highlight')}}
...
<div className=' font-semibold text-md p-2 px-3 line-clamp-2'>
{item?.snippet?.title}
</div>
    </div>
  )
